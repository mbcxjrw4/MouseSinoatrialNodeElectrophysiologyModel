...
int main()
{
    ...
    in_1 = fopen ("xinzhao2D_new.txt", "r");
    for (z = 0; z < Z; z++)
    {
        for (x = 0; x < X; x++)
        {
    ...
    in_3 = fopen ("diffusion_geo.txt", "r");
    for (z = 0; z < Z; z++)
    {
        for (x = 0; x < X; x++)
        {
...
/*    FILE *diff_vtk;
    ...
    for(tout = ddt; tout<total_time; tout+=ddt)
    {
        ...
        #ifdef PARALLEL         
        #pragma omp parallel for default(none) shared(new_para, cell, D, tout, t, y, ddt) private(x , z, du)
        #endif
        for(x = 1; x < X-1; x++)
            for(z = 1; z < Z-1; z++)
            {
                if(cell[x][z]->cell_type>0)
                    {
                        // CVode(cell[x][z]->cvode_mem, tout, y[x][z], &t, CV_NORMAL); // 1 time step solution.

                        du = DIFFUSION(x, z, D, cell);

                        new_para[x][z] = cell[x][z]->V + (ddt/2)*du;
                    }
            } // end of for loop
                ...
                if(cell[x][z]->cell_type>0)
                    {
                        ...
                        new_para[x][z] = cell[x][z]->V + ddt*cell[x][z]->dvdt + ddt*du;
                    }
            } // end of for loop
